...
const AboutMe: FC = () => {
    ...
    const slideContentLst: JSX.Element[] = [
        <Accordion title="Conoceme" classNames={{ header: 'w-full', content: 'flex flex-row flex-wrap justify-center' }}>
            <p className="m-4 md:w-1/2 w-full">¡Hola! Soy Julio Castro, un ingeniero de software con una profunda pasión por la escalada, el alpinismo y la ciencia. Mi vida se define por el equilibrio entre la innovación tecnológica y la conexión con la naturaleza. Cada línea de código que escribo y cada montaña que escalo son reflejo de mi compromiso con la superación personal y profesional.</p>
        </Accordion>,
        <Accordion title="Mi Trayectoria y Logros" classNames={{ header: 'w-full', content: 'flex flex-row flex-wrap justify-center' }}>
            <ul className="m-4 md:w-1/2 w-full">
        ...
        <Accordion title="Mi Filosofía de Vida" classNames={{ header: 'w-full', content: 'flex flex-row flex-wrap justify-center' }}>
            <p className="m-4 md:w-1/2 w-full">Creo firmemente que cada desafío, ya sea en el mundo digital o en la inmensidad de las montañas, es una oportunidad para aprender, crecer y transformar. La clave está en no detenerse nunca, en buscar siempre esa "cima" que nos motiva a ser mejores.</p>
        </Accordion>,
        <Accordion title="Misión y Visión" classNames={{ header: 'w-full', content: 'flex flex-row flex-wrap justify-center' }}>
            <ul className="m-4 md:w-1/2 w-full">
